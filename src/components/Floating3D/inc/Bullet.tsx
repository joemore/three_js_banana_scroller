 ...
 * @returns 
 */
function Bullet({ index, z, speed } : any) {
  ...
  const { scene, nodes, materials } : any = useGLTF('/bullet.gltf')
...
export default function Bullets({ speed = 1, count = 80, depth = 80, easing = (x : number) => Math.sqrt(1 - Math.pow(x - 1, 2)) }) {
  return (
    ...
    <Canvas gl={{ antialias: false }} dpr={[1, 1.5]} camera={{ position: [0, 0, 10], fov: 20, near: 0.1, far: depth + 15 }}>
      ...
      {Array.from({ length: count }, (_, i) => <Bullet key={i} index={i} z={Math.round(easing(i / count) * depth)} speed={speed} /> /* prettier-ignore */)}
